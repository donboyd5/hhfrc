...
setdiff(names(df), lookup)

newvars <- tibble(neon = setdiff(names(df), lookup)) |>
  filter(!str_starts(neon, fixed("..."))) |>
  mutate(
    tentative = str_to_lower(neon),
    tentative = str_remove_all(tentative, "\\?"),
    tentative = str_remove_all(tentative, "\\."),
    tentative = str_remove_all(tentative, "\\-"),
    tentative = str_remove_all(tentative, "\\(c\\)"),
    tentative = str_remove_all(tentative, "\\(f\\)"),
    tentative = str_replace(tentative, "phone ", "phone"),
    tentative = str_replace(tentative, "line ", "line"),
    tentative = str_trim(tentative),
    tentative = str_replace_all(tentative, " ", "_")
  )

writexl::write_xlsx(
  newvars,
  fs::path(work, "newnames.xlsx")
)
