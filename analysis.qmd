...
vnames <- read_csv(
  ...
  fullname, Full Name (F), donor
  company_name, Company Name, donor
  city, City, donor
...
acct2 <- acct1 |>
  ...
  select(
    ...
    accountid = 2,
    hhid = 3,
    actcontactid = 4, # drop, .02% complete
    activityid = 5, # drop, .02% complete
    companyid = 6,
    groupid = 7,
    importid = 8, # drop, only 2 values, don't know what it is
    hhname = 9,
    fullname = 10,
    companyname = 11,
    fulladdress = 12,
    ...
    email1 = 15,
    contact_type = 16,
    donotcontact = 17, # make logical
    deceased = 18, # make logical
    acctnote = 19,
    partnerchild = 20,
    regtotal = 21, # make numeric
    donatetotal = 22, # make numeric
    gender = 23, # drop 98% missing
    junk = 24 # drop all missing
  )

glimpse(acct2)
skim(acct2)

count(acct2, accttype)
#   accttype       n
#   <chr>      <int>
# 1 Company      291
# 2 Individual  4511

count(acct2, importid)
count(acct2, accttype, contact_type) # 8 company types, 2 individual types, but 11 of 12 are in individual account types
count(acct2, donotcontact)
count(acct2, deceased)

acct3 <- acct2 |>
  select(-c(actcontactid, activityid, importid, gender, junk)) |>
  mutate(
    donotcontact = donotcontact == "Yes",
    deceased = deceased == "Yes",
    regtotal = as.numeric(regtotal),
    donatetotal = as.numeric(donatetotal)
  )
...
donors2 <- donors1 |>
  ...
  select(-...15) |>
  mutate(donation = as.numeric(amount), date = lubridate::mdy(date))
...
don1 <- donations |>
  ...
  left_join(accounts |> select(accountid, accttype, hhid, hhname), by = join_by(accountid)) |> 
  filter(accttype=="Individual") |> 
  mutate(hhfy=ifelse(month(date)>=7, year(date)+1, year(date)),
...
donfy <- don1 |> 
  summarise(donation=sum(donation),
            .by=c(hhid, hhname, hhfy))
