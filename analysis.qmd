...
acctlink <- accounts |>
  ...
  mutate(
    contact_type = case_when(
      household_id == "54" & account_id == "34713" ~
        "Primary Household Contact", # Cora May Howe
...
# special household_id drops related to Judy
...
idvars <- c("id", "name", "group", "household_id", "hh_name", "account_id")

count(don1, campaign_id, campaign_name)

hhalldon <- don1 |>
  group_by(id, name) |>
  mutate(
    donation_all = sum(donation_amount),
    donation_last5years = sum(donation_amount[hhfy %in% 2021:2025]),
    donation_max = max(donation_amount),
    n_max_donation = sum(donation_amount == donation_max, na.rm = TRUE),
    date_of_max = donation_date[donation_amount == donation_max][1],
    campaign_of_max = campaign_name[donation_amount == donation_max][1],
    last_donation_date = max(donation_date),
    fy_of_last_donation = hhfy[donation_date == last_donation_date][1],
    total_donation_latest_donation_fy = sum(donation_amount[
  ...
  left_join(
    contacts |>
      select(
        ...
        address_line1:primary_contact_phone
      ),
    by = join_by(id, name)
  ) |>
  filter(!deceased, account_id != "36805") |>
  select(-deceased)

ns(hhall_enhanced)

saveRDS(hhall_enhanced, fs::path(rds, "hhall_enhanced.rds")) # djb
...
don1_2021plus <- don1 |>
  filter(account_type == "Individual") |>
  mutate(donation_all = sum(donation_amount), .by = c(id, name)) |>
...
events_data <- events |>
  ...
  arrange(event_majorcat, event_category, event_id, event_name)


# 10/16/2025 djb CAUTION: DO NOT RUN THIS UNTIL I FIGURE OUT WHAT IT IS DOING AS IT WOULD OVERWRITE GOOGLE SHEET
# range_write(
#   ss,
#   events_data,
...
# djb create registrations2 to make up for bad names -- TEMPORARY FIX ----
registrations2 <- registrations |>
  rename(event_status = status)
count(registrations2, event_status)
...
# temporary name fix create hhall_enhanced2 ----
hhall_enhanced2 <- hhall_enhanced |>
  rename(hh_name = name)

dnc <- hhall_enhanced2 |>
  filter(do_not_contact, donation_all >= 1000) |>
  mutate(explanation = "") |>
  relocate(explanation, .after = hh_name)
...
# djb further comment out google sheets to avoid accidental overwrite ----
# range_write(
#   ss,
#   top100,
...
#  acccount id	name 36805	Cash and Credit Card Miscellaneous Transactions in 2021 plus was 28 transactions, $3890, all do_not_contact

regs1 |> filter(is.na(id))

comp <- don1 |>
  left_join(regs1, by = join_by(household_id, hh_name)) |>
...
don1 <- donations |>
  filter(account_type == "Individual") |>
  # filter(!deceased, !do_not_contact) |> # these pertain to the individual account
  filter(
    donation_type %in% c("DONATION", "PLEDGEPAYMENT"),
    donation_status == "SUCCEEDED"
  ) |>
...
acctlink <- accounts |>
  ...
  mutate(
    contact_type = case_when(
      household_id == "54" & accountid == "34713" ~ "Primary Household Contact", # Cora May Howe
      household_id == "54" & accountid == "34712" ~ "Household Contact", # Robert Howe, deceased and DNC
      .default = contact_type
    )
  ) |>
  # filter(account_type=="Individual", group=="account", id=="38532")
...
ddnc <- don3 |>
  filter(deceased | do_not_contact)
...
don4 <- don3 |>
  ...
  select(-deceased, -do_not_contact) |>
