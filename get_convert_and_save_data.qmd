...
linkages <- accounts |>
  ...
  select(hhid, hh_name, accountid, full_name, full_address, city, state) |>
...
donations <- donations_no_hhid |>
  left_join(
    linkages |> select(hh_name, hhid, accountid),
  ...
  select(id, name, group, hhid, hh_name, accountid, full_name, everything())
...
## Put eventid, event_code, event_majorcat, ..., hhid, and hh_name on registrations file
...
regsid <- regsnoid |>
  ...
  left_join(
    linkages |> select(accountid, account_name = full_name, hhid, hh_name),
