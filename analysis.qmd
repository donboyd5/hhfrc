...
xljudy <- read_excel(fs::path(judy, "HH Donor List - All Donations $15+.xlsx"), sheet="List by Total Donations")
...
df <- vroom(fs::path(neon, "HH Donor List - All Donations $15+.csv"), col_types = cols(.default = col_character()))
ns(df)
...
don1 <- donations |> 
  ...
  filter(donation_type %in% c("DONATION", "PLEDGEPAYMENT"), donation_status=="SUCCEEDED") |> 
  # special drops related to Judy
  filter(!hhid %in% 
           c("224")) # Caroline Eastman
...
acctlink <- accounts |> 
  ...
  mutate(contact_type = case_when(hhid=="54" & accountid=="34713" ~ "Primary Household Contact", # Cora May Howe
  ...
  filter(account_type=="Individual") |> 
  filter(group=="account" |
           (group=="household" & str_detect(contact_type, "Primary Household Contact"))) |> 
  # captures PHC in combination with anything else CAUTION: keeps company contacts
  select(id, name, group, primary_contact=fullname, contact_type, deceased, donotcontact)
...
donations |> filter(id=="21") |> select(id, name, donation, donation_date, payment_status) |> janitor::adorn_totals()
donations |> filter(id=="21") |> select(id, name, donation, donation_date, payment_status) |> arrange(donation_date) |> janitor::adorn_totals()
# this explains it: 21 Wendy Duvall   100.00    2018-09-18       CANCELED

```


## Great now that we know how to get donations, we do Alix's tables
...
hhalldon <- don1 |> 
  group_by(id, name) |> 
  mutate(donation_all=sum(donation),
         ...
         total_donation_latest_donation_fy = sum(donation[hhfy==fy_of_last_donation])
         ) |> 
  slice_head(n = 1) |> 
  ungroup() |> 
  arrange(desc(donation_all), id) |> 
  select(any_of(idvars), donation_all, donation_last5years, donation_max, n_max_donation, date_of_max, campaign_of_max,
         last_donation_date, fy_of_last_donation, total_donation_latest_donation_fy)
...
hhall_enhanced <- hhalldon |> 
  left_join(acctlink |> select(-c(group)), 
by = join_by(id, name)) |>
left_join(contacts |> select(id, name, pfname, plname, address1:pphone), by = join_by(id, name)) |> 
...
hhalldon_2021plus <- don1_2021plus |> 
  group_by(id, name) |> 
  mutate(donation_last5years = sum(donation[hhfy %in% 2021:2025]),
         donation_max=max(donation),
         n_max_donation = sum(donation == donation_max, na.rm = TRUE),
         date_of_max = donation_date[donation == donation_max][1],
         campaign_of_max = campaign[donation == donation_max][1],
...
events_data <- events |> 
  select(event_majorcat, event_category, event_name, event_start_date, eventid, event_code) |> 
...
registrations |> 
  filter(type %in% c("Registrant", "Registrant & Attendee"),
    event_status=="SUCCEEDED") |> pull(amount) |> sum()
...
don1 <- donations |>
  ...
  select(id, name, group, hhid, hhname, accountid, fullname, donation_date, campaign, fund, purpose, donation) |> # campaign, fund, purpose
  mutate(hhfy = ifelse(month(donation_date) >= 7, year(donation_date) + 1, year(donation_date))) |> 
  mutate(last_donation_date=max(donation_date, .by=id, name)
skim(don1)
...
donfinal <- donsums |>
  ...
  left_join(
    donfywide |>
      select(id, name, `2021`:`2026`),
    by = join_by(id, name)
  ) |>
  # left_join(donations |> select(id, name, deceased, donotcontact) |> distinct(), by = join_by(id, name)) |> 
  arrange(desc(total)) |> 
  mutate(cumpct_total = cumsum(total) / sum(total)) |>  
  arrange(desc(since_2021)) |> 
  mutate(cumpct_2021plus = cumsum(since_2021) / sum(since_2021)) |> 
  relocate(cumpct_total, cumpct_2021plus, .after = since_2021) 
donfinal
# cum_percent = 100 * cumsum(your_variable) / sum(your_variable)
...
range_write(
  ss,
  donfinal |> select(-cumpct_total) |> arrange(desc(since_2021)),
  sheet = "all_donors_2021plus_sort",
  range = "A13",
  col_names = TRUE,
  reformat = FALSE
)
...
range_write(
  ss,
  donfinal |> select(-cumpct_2021plus) |> arrange(desc(total)),
  sheet = "all_donors_allyears_sort",
  range = "A13",
  col_names = TRUE,
  reformat = FALSE
)

count(events, event_majorcat)
catorder <- c("performance", "class", "community", "other")

events_data <- events |> 
  select(event_majorcat, event_category, event_name, event_start_date, eventid, event_code) |> 
  mutate(event_majorcat=factor(event_majorcat, levels=catorder)) |> 
  arrange(event_majorcat, event_category, eventid, event_name)
