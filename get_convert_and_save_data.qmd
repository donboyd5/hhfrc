...
```{r}
#| label: household-contact-information
...
contacts3 <- contacts2 |>
  mutate(
    id = ifelse(is.na(household_id), account_id, household_id),
    name = household_name_account_name,
    group = case_when(
      !is.na(household_id) ~ "household",
      !is.na(account_id) ~ "account",
      .default = "other"
    )
  ) |>
...
count(acct2, import_id) # drop its only values are 10 and 4 with most missing
count(acct2, do_not_contact)
count(acct2, deceased)
ns(acct2)

drops <- c(
  "activity_contact_account_id",
  "activity_id",
  "fname",
  "gender",
  "import_id"
...
acct3 <- acct2 |>
  ...
  mutate(
    id = ifelse(is.na(household_id), account_id, household_id),
    name = ifelse(is.na(household_name), full_name, household_name),
    group = case_when(
      !is.na(household_id) ~ "household",
      !is.na(account_id) ~ "account",
      .default = "other"
...
linkages <- accounts |>
  ...
  arrange(as.integer(household_id), as.integer(account_id))
...
don3 <- don2 |>
  ...
  mutate(
    ...
    donation_amount = parse_number(donation_amount),
    deceased = deceased == "Yes",
    do_not_contact = do_not_contact == "Yes"
  )
...
glimpse(don3)
skim(don3)
check <- don3 |> filter(is.na(donation_amount)) # payment_status is DEFERRED
count(check, payment_status)
count(don3, payment_status)
count(don3, donation_status)

saveRDS(don3, fs::path(rds, "donations_no_household_id.rds"))
...
donations <- donations_no_household_id |>
  left_join(
    linkages |> select(household_name, household_id, account_id),
    by = join_by(household_name, account_id)
  ) |>
  mutate(
    id = ifelse(is.na(household_id), account_id, household_id),
    name = ifelse(is.na(household_name), full_name, household_name),
...
names(ev2)
glimpse(ev2)
count(ev2, topic) # has names of people for some events -- KEEP
count(ev2, code) # don't know what it is, but not blank, appears to be a date -- KEEP
count(ev2, attendees) # should be integer
count(ev2, waitlisted) # "0" for all -- DROP
count(ev2, starts_on) # mm/dd/yyyy
count(ev2, ends_on) # mm/dd/yyyy
count(ev2, status) # ACTIVE or INACTIVE -- KEEP
skim(ev2)
...
ev3 <- ev2 |>
  ...
  mutate(
    ...
    starts_on = lubridate::mdy(starts_on),
    ends_on = lubridate::mdy(ends_on)
  )
glimpse(ev3)
skim(ev3)
ev3 |> filter(attendees == 406)
count(ev3, category) # 43 categories; NA has 15 records
...
regs3 <- regs2 |>
  mutate(
    across(c(created_date, last_modified_date, registered_on, starts_on), \(x) {
      lubridate::mdy(x)
    }),
...
## Put eventid, event_code, event_majorcat, ..., household_id, and household_name on registrations file

```{r}
#| label: add-eventid-to-registrations
...
regsid <- regsnoid |>
  ...
  left_join(
    ...
    by = join_by(account_id, account_name)
  ) |>
  mutate(
    id = ifelse(is.na(household_id), account_id, household_id),
    name = ifelse(is.na(household_name), account_name, household_name),
    group = case_when(
      !is.na(household_id) ~ "household",
      !is.na(account_id) ~ "account",
      .default = "other"
...
unique(check$event) # the missing ones all have 2025 registraions after the data import!
count(regsid, type)
...
check |>
  ...
  mutate(year = year(starts_on)) |> # djb
  summarise(n = n(), amount = sum(amount), .by = year) |>
  janitor::adorn_totals()

check |>
  filter(!is.na(account_id)) |>
  arrange(desc(last_modified_date)) |> # djb
  filter(row_number() <= 24672) |>
  # mutate(year = year(event_start_date)) |>
  mutate(year = year(starts_on)) |>
  arrange(year) |>
  summarise(n = n(), amount = sum(amount), .by = year) |>
  janitor::adorn_totals()
...
check |>
  mutate(accountid_missing = is.na(account_id)) |>
...
check |>
  ...
  filter(row_number() <= 24672) |>
  mutate(has_accountid = !is.na(account_id)) |>
