...
#  [1] "accountid"          "account_name"        "start_date"         "Payment"
#  [8] "Created Date"       "Created By"         "Last Modified Date" "Last Modified By"   "Type"               "System Updates"     "Campaign"
# [15] "Registered On"      "status"             "...17"

# CAUTION: note that event name is called Event in the Events file, and Event Name in the registrations file. The code below deals properly with this.

vnames <- read_csv(
  "vname, neon, sources  
  hhid, Household ID, accounts
  hhname, Household Name, donor-accounts
  hhacctname, Household Name/Account Name, hhreport
  ...
  notes, Notes, regs
  amount, Amount, regs
  payment_method, Payment, regs
  created_date, Created Date, regs
  created_by, Created By, regs
  last_mod_date, Last Modified Date, regs
  last_mod_by, Last Modified By, regs
  type, Type, regs
  sysupdt, System Updates, regs
  campaign, Campaign, regs
  registered_date, Registered On, regs
  ...
  fund, Fund, donor
  donation_status, Donation Status, donor
  payment_status, Payment Status, donor
  
  donation_type, Donation Type, donor
  individual_type, Individual Type, donor
  tribute_type, Tribute Type, donor
  tender_type, Tender Type, donor

  fullname, Full Name (F), donor-accounts
  salutation, Household Salutation / Preferred Name, hhreport
  city, City, donor
  address1, Address Line 1, donor
  address2, Address Line 2, hhreport
  state, State/Province, donor
  zip, Zip Code, donor
  fulladdress, Full Street Address (F), accounts
  email1, Email 1, accounts
  pemail, Primary Contact Email, hhreport
  pfname, Primary Contact First Name, hhreport
  plname, Primary Contact Last Name, hhreport
  pphone, Primary Contact Phone, hhreport
...
writexl::write_xlsx(vnames, fs::path(work, "vnames.xlsx"))
...
contacts3 <- contacts2 |>
  select(-...14) |>
  mutate(
    id = ifelse(is.na(hhid), accountid, hhid),
    name = hhacctname,
    group = case_when(
      !is.na(hhid) ~ "household",
...
acct3 <- acct2 |>
  ...
  mutate(
    id = ifelse(is.na(hhid), accountid, hhid),
    name = ifelse(is.na(hhname), fullname, hhname),
    group = case_when(
      !is.na(hhid) ~ "household",
      !is.na(accountid) ~ "account",
      .default = "other"
...
donations <- donations_no_hhid |>
  ...
  select(id, name, group, hhid, hhname, accountid, fullname, everything())
...
## Put eventid, event_code, event_majorcat, ..., hhid, and hhname on registrations file
...
regsid <- regsnoid |>
  ...
  mutate(
    id = ifelse(is.na(hhid), accountid, hhid),
    name = ifelse(is.na(hhname), account_name, hhname),
    group = case_when(
      !is.na(hhid) ~ "household",
      !is.na(accountid) ~ "account",
      .default = "other"
