...
vnames <- read_csv(
  ...
  event_start, Starts On, events
  event_end, Ends On, events
  event_status, Status, events
  ...
  mutate(
    event_majorcat = case_when(
      ...
      event_category == "Children's Theater" &
        ...
        "performance",
      event_name == "An Unforgettable Crooner Cabaret" ~ "performance",
      ...
      event_category %in% c("Auditions") ~ "other",
...
Currently this comes from exporting Events data. Done for 2 time periods separately because of neon limits on # of records that can be exported -- 2011 through 2019 calendar years, and 2020 forward.

TODO: Look for whether it would be better to create a report for this.

```{r}
#| label: registrations-from-expot
#| eval: false
