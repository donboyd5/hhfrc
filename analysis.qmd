...
df1 <- donations |> 
  right_join(accounts |> filter(type=="Individual") |> select(accountid, acctname2=acctname), by = join_by(accountid))
  filter(type=="DONATION") |> # exclude pledges
  summarise(amount = sum(amount), .by=c(accountid, acctname))

df2 <- donations |> 
  filter(type=="DONATION") |>  # exclude pledges
  left_join(accounts |> 
    select(accountid, acctname2=acctname, atype=type),
    by = join_by(accountid)) |>
      filter(atype=="Individual") |> 
    summarise(amount = sum(amount), .by=c(accountid, acctname)) |> 
  arrange(desc(amount))
